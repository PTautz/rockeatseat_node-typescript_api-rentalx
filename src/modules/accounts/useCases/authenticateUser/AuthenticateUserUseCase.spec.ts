...
describe("Authenticate user", () => {
  ...
  it("should not be possible to authenticate an nonexistent user", async () => {
    await expect(
      authenticateUserUseCase.execute({
        email: "false@email.com",
        password: "1234",
      }),
    ).rejects.toEqual(new CustomError(401, "Email or password incorrect!"));
  });
